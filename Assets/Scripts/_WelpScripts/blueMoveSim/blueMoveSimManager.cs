...
public class blueMoveSimManager : MonoBehaviour
{
    public static blueMoveSimManager instance;
    public string gName = "blueMoveSim";
    ...
    public float TotalNumofAttmpts;
    ...
    void Start()
    {
        ...
        //{
        //    actions.Add(testList[i], moveCarFurther);
        //}
    ...
    public void startGame()
    {
        ...
        if (isFirstLevel)
        {
            ...
            _blinkWord.keywordText[3].text = wordsToSay[0].text;
        }
    ...
    void moveCarFurther()
    {
        ...
        car.transform.position = firstPoints[postionIndex].position;
        car.transform.rotation = firstPoints[postionIndex].rotation;
        _animator.SetBool("rotateTires", true);
        translateCarTo(finalPoints[postionIndex]);
        postionIndex++;
        startVarCounting();
        StartCoroutine(endMovingcar_coroutine());
    }

    void translateCarTo(Transform to)
    {
        car.transform.LeanMove(to.position, timeToReachPointCar);
    }

    IEnumerator endMovingcar_coroutine()
    {
        yield return new WaitForSeconds(timeToReachPointCar);

        numOfGoodAttmpts++;
        _animator.SetBool("rotateTires", false);

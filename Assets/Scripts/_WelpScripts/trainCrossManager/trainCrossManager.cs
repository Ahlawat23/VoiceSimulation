...
public class trainCrossManager : MonoBehaviour
{
    public static trainCrossManager instance;
    public string gName;
    ...
    public List<Transform> train1Tracks = new List<Transform>();
    public List<Transform> train2Tracks = new List<Transform>();
    public List<blinkWord> blinkWords = new List<blinkWord>();


    [Header("game vars")]
    public int fountainIndex;
    public float train1YOffset;
    public float train2YOffset;
    ...
    public void StartGame()
    {
        ...
        Time.timeScale = 1;
        blinkWords[0].switchText(0);
        blinkWords[1].switchText(0);
    ...
    IEnumerator waitFroEnd()
    {
        ...
        if (Train1PostionIndex >= train1Tracks.Count && Train2PostionIndex >= train2Tracks.Count)
            GameOver();
    }

    public int Train1PostionIndex;
    void moveGas()
    {

        if (Train1PostionIndex < train1Tracks.Count)
        {
            Debug.Log("moving");
            
            Vector3 mov = new Vector3(train1Tracks[Train1PostionIndex].position.x, train1Tracks[Train1PostionIndex].position.y + train1YOffset, train1Tracks[Train1PostionIndex].position.z);
            train1.transform.LeanMove(mov, 0.3f);
    ...
    public int Train2PostionIndex;
    void moveCoal()
    {
        if (Train2PostionIndex < train2Tracks.Count)
        {
            Debug.Log("moving");

            Vector3 mov = new Vector3(train2Tracks[Train2PostionIndex].position.x, train2Tracks[Train2PostionIndex].position.y + train2YOffset, train2Tracks[Train2PostionIndex].position.z);
            train2.transform.LeanMove(mov, 0.3f);
